...
namespace TelventDMS.Services.NetworkModelService.TestClient.Tests
{
    public class TestGda : IDisposable
    {
        ...
        private NetworkModelGDAProxy query
        {
            get
            {
        ...
        public ResourceDescription GetValues(long globalId)
        {
            ...
            try
            {
                ...
                rd = query.GetValues(globalId, properties);
        ...
        public List<long> GetExtentValues(ModelCode modelCode)
        {
            ...
            try
            {
                ...
                iteratorId = query.GetExtentValues(modelCode, properties);
        ...
        public void ReadAllPowerTransformers()
        {
            XmlTextWriter writer = null;
            int iteratorId = 0;
            int resNum = 2;
            int resourcesLeft = 0;
            ModelCode powerTransformerType = ModelCode.POWERTR;

            try
            {
                List<ModelCode> ids = modelResourcesDesc.GetAllPropertyIds(powerTransformerType);

                iteratorId = query.GetExtentValues(powerTransformerType, ids);
                resourcesLeft = query.IteratorResourcesLeft(iteratorId);

                writer = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\AllPowerTransformers.xml", Encoding.Unicode);
                writer.Formatting = Formatting.Indented;

                readResourcesAndWriteToXML(writer, iteratorId, resNum, resourcesLeft);

                query.IteratorClose(iteratorId);

                Console.WriteLine("Reading succ");
            }
            catch (Exception e)
            {
        ...
        private void readResourcesAndWriteToXML(XmlTextWriter writer, int iteratorId, int resNum, int resourcesLeft)
        {
            while (resourcesLeft > 0)
            {
                List<ResourceDescription> rds = query.IteratorNext(resNum, iteratorId);

                for (int i = 0; i < rds.Count; i++)
                {
                    rds[i].ExportToXml(writer);
                    writer.Flush();
                }
        ...
        public void ReadTransformerWindingLowestGID()
        {
            XmlTextWriter writer = null;
            int iteratorId = 0;
            List<long> ids = new List<long>();
            int resNum = 2;
            int resourcesLeft = 0;
            ModelCode powertrModelCode = ModelCode.POWERTR;

            try
            {
                List<ModelCode> props = modelResourcesDesc.GetAllPropertyIds(powertrModelCode);

                iteratorId = query.GetExtentValues(powertrModelCode, props);
                resourcesLeft = query.IteratorResourcesLeft(iteratorId);
            ...
            association.PropertyId = ModelCode.POWERTR_WINDINGS;
            association.Type = ModelCode.POWERTRWINDING;
            ...
            try
            {
                ...
                iteratorId = query.GetRelatedValues(gid, properties, association);
                resourcesLeft = query.IteratorResourcesLeft(iteratorId);

                writer = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\TransformerWinding.xml", Encoding.Unicode);
                writer.Formatting = Formatting.Indented;
        ...
        public List<long> TestGetExtentValuesAllTypes()
        {
            ...
            try
            {
                foreach (DMSType type in Enum.GetValues(typeof(DMSType)))
                {
                    ...
                    iteratorId = query.GetExtentValues(modelResourcesDesc.GetModelCodeFromType(type), properties);
                    int count = query.IteratorResourcesLeft(iteratorId);
                    ...
                    while (count > 0)
                    {
                        ...
                        count = query.IteratorResourcesLeft(iteratorId);
                    }

                    bool ok = query.IteratorClose(iteratorId);
        ...
        public UpdateResult TestApplyDeltaInsert()
        {
            ...
            try
            {
                ...
                updateResult = query.ApplyUpdate(delta);
