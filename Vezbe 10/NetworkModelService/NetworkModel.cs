	...
	public class NetworkModel
	{
		...
		private void DeleteEntity(ResourceDescription rd)
		{
			...
			try
			{
				...
				if (io.IsReferenced)
				{
					// to do add referenced entities
					string message = String.Format("Failed to delete entity (GID = 0x{0:x16}) because it is referenced by entities with GIDs: {1}.", globalId, 0);
					CommonTrace.WriteTrace(CommonTrace.TraceError, message);
					throw new Exception(message);
				}
