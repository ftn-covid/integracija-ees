...
{	
	...
	public class ModelResourcesDesc
	{
		...
		/// <summary>
		/// Mapping of ModelCode enums to name of classes/attributes.
		/// </summary>
		private Dictionary<ModelCode, string> modelCode2Name = new Dictionary<ModelCode, string>(new ModelCodeComparer());

		/// <summary>
		/// Mapping of name of classes/attributes to ModelCode enums.
		/// </summary>
		private Dictionary<string, ModelCode> name2modelCode = new Dictionary<string, ModelCode>();

		private HashSet<DMSType> catalogs = new HashSet<DMSType>(new DMSTypeComparer());
		...
		/// <summary>
		/// Gets number of model types.
		/// </summary>
		public int Count
		{
			get { return this.resourceDescs.Count; }
		}
		...
		public static DMSType GetMaxDMSType()
		{
			return DMSType.WINDINGTEST;
		}
		...
		public string GetAttributeNameFromModelCode(ModelCode classId)
		{
			string name = null;
			if (this.modelCode2Name.TryGetValue(classId, out name))
			{
				return name;
			}
			else
		...
		public ModelCode GetModelCodeFromAttributeName(string name)
		{
			ModelCode modelCode;
			if (this.name2modelCode.TryGetValue(name, out modelCode))
			{
				return modelCode;
			}
			else
			{
				...
				string propertyFullName = string.Empty;

				if (splitResult.Length > 1 && name2modelCode.ContainsKey(splitResult[0]))
				{
					ModelCode classId = name2modelCode[splitResult[0]];
					List<ModelCode> ancestors = null;
					this.GetResourceAncestors(classId, ref ancestors);

					for (int i = ancestors.Count - 1; i >= 0; i--)
					{
						if (modelCode2Name.ContainsKey(ancestors[i]))
						{
							propertyFullName = string.Format("{0}.{1}", modelCode2Name[ancestors[i]], splitResult[1]);
							if (name2modelCode.ContainsKey(propertyFullName))
							{
								return name2modelCode[propertyFullName];
							}
						}
					}
				}
		...
		{
			modelCode2Name[ModelCode.IDOBJ] = "IdentifiedObject";
			modelCode2Name[ModelCode.IDOBJ_GID] = "IdentifiedObject.globalId";			

			# region Initialize name 2 model code map

			foreach (KeyValuePair<ModelCode, string> pair in modelCode2Name)
			{
				//name2modelCodeNo[pair.Value] = pair.Key;
				name2modelCode.Add(pair.Value, pair.Key);
			}

			# endregion Initialize name 2 model code map
		}
		...
		/// <summary>
		/// Returns a model code representing reference target
		/// </summary>		
		/// <param name="referencingProperty">model code of the reference-type property, applicable to referencing entity</param>
		/// <returns>model code representing reference vector property, applicable to the referenced entity</returns>
		public ModelCode GetTargetingProperty(ModelCode referencingProperty)
		{
			switch (referencingProperty)
			{								
				case ModelCode.PSR_LOCATION: return ModelCode.LOCATION_PSRS;
				case ModelCode.LOCATION_PSRS: return ModelCode.PSR_LOCATION;

				case ModelCode.CONDEQ_BASVOLTAGE: return ModelCode.BASEVOLTAGE_CONDEQS;
				case ModelCode.BASEVOLTAGE_CONDEQS: return ModelCode.CONDEQ_BASVOLTAGE;

				case ModelCode.POWERTRWINDING_POWERTRW: return ModelCode.POWERTR_WINDINGS;
				case ModelCode.POWERTR_WINDINGS: return ModelCode.POWERTRWINDING_POWERTRW;				

				case ModelCode.WINDINGTEST_POWERTRWINDING: return ModelCode.POWERTRWINDING_TESTS;
				case ModelCode.POWERTRWINDING_TESTS: return ModelCode.WINDINGTEST_POWERTRWINDING;			

				default:
					throw new Exception(string.Format("Failed to retrieve property holding a target corresponding the reference: {0}.", referencingProperty));
