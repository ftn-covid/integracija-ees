...
namespace FTN.Common
{
	...
	public class Property : IComparable
	{
		...
		public Property(Property toCopy)
		{
			...
			PropertyType type = GetPropertyType(id);
			switch (type)
			{
				...
				case PropertyType.Reference:
					this.value = (PropertyValue)new LongPropertyValue((LongPropertyValue)toCopy.value);
					break;
				case PropertyType.Float:
					this.value = (PropertyValue)new FloatPropertyValue((FloatPropertyValue)toCopy.value);
					break;
				case PropertyType.Double:
					this.value = (PropertyValue)new DoublePropertyValue((DoublePropertyValue)toCopy.value);
					break;
				case PropertyType.String:
					this.value = (PropertyValue)new StringPropertyValue((StringPropertyValue)toCopy.value);
					break;				
				...
				case PropertyType.DateTimeVector:
					this.value = (PropertyValue)new LongPropertyValues((LongPropertyValues)toCopy.value);
					break;
				case PropertyType.FloatVector:
					this.value = (PropertyValue)new FloatPropertyValues((FloatPropertyValues)toCopy.value);
					break;
				case PropertyType.StringVector:
					this.value = (PropertyValue)new StringPropertyValues((StringPropertyValues)toCopy.value);
					break;				
			}
		}
		...
		public Property(ModelCode id)
		{
			...
			switch (type)
			{
				...
				case PropertyType.DateTime:
				case PropertyType.Reference:
					this.value = (PropertyValue)new LongPropertyValue();
					break;
				case PropertyType.Float:
					this.value = (PropertyValue)new FloatPropertyValue();
					break;
				case PropertyType.Double:
					this.value = (PropertyValue)new DoublePropertyValue();
					break;
				case PropertyType.String:
					this.value = (PropertyValue)new StringPropertyValue();
					break;				
				case PropertyType.ReferenceVector:
				case PropertyType.BoolVector:
				case PropertyType.ByteVector:
				...
				case PropertyType.TimeSpanVector:
				case PropertyType.DateTimeVector:
					this.value = (PropertyValue)new LongPropertyValues();
					break;
				case PropertyType.FloatVector:
					this.value = (PropertyValue)new FloatPropertyValues();
					break;
				case PropertyType.StringVector:
					this.value = (PropertyValue)new StringPropertyValues();
					break;				
			}
		}
		...
		public Property(ModelCode id, double doubleValue)
		{
			this.id = id;
			this.value = (PropertyValue)new DoublePropertyValue();
		...
		public static Property CreateFromString(ModelCode propId, string stringValue)
		{
			...
			switch (propType)
			{
				...
				case PropertyType.Float:
					...
					break;
				case PropertyType.Double:
					property = new Property(propId, double.Parse(stringValue));
					break;
				case PropertyType.String:
		...
		public static List<PropertyValue> GetValuesAsList(PropertyValue value)
		{
			List<PropertyValue> retVal = new List<PropertyValue>();

			if (value.GetType() == typeof(LongPropertyValues) && value.LongValues != null && value.LongValues.Count > 0)
			{
				for (int i = 0; i < value.LongValues.Count; i++)
				{
					PropertyValue newValue = (PropertyValue)new LongPropertyValue();
					newValue.LongValue = value.LongValues[i];
					retVal.Add(newValue);
				}
			}
			else if (value.GetType() == typeof(FloatPropertyValues) && value.FloatValues != null && value.FloatValues.Count > 0)
			{
				for (int i = 0; i < value.FloatValues.Count; i++)
				{
					PropertyValue newValue = (PropertyValue)new FloatPropertyValue();
					newValue.FloatValue = value.FloatValues[i];
					retVal.Add(newValue);
				}
			}
			else if (value.GetType() == typeof(StringPropertyValues) && value.StringValues != null && value.StringValues.Count > 0)
			{
				for (int i = 0; i < value.StringValues.Count; i++)
				{
					PropertyValue newValue = (PropertyValue)new StringPropertyValue();
					newValue.StringValue = value.StringValues[i];
					retVal.Add(newValue);
				}
			}			
			else
		...
		public override string ToString()
		{
			switch (this.Type)
			{
				...
				case PropertyType.Double:
					return this.AsDouble().ToString(new System.Globalization.CultureInfo("en-US", false).NumberFormat);
				case PropertyType.String:
		...
		public void SetValue(double doubleValue)
		{
			...
			{
				string errorMessage = String.Format("Failed to set value for property ({0}) because specified value ({1}) is not compatible with property type ({2}).", this.id, doubleValue, type);
				CommonTrace.WriteTrace(CommonTrace.TraceError, errorMessage);
				throw new Exception(errorMessage);
			}
		}
		...
		public double AsDouble()
		{
			...
			{
				string errorMessage = String.Format("Failed to return value from property for specified type ({0}) because it is incompatible with actual property type ({1})", PropertyType.Double, type);
				CommonTrace.WriteTrace(CommonTrace.TraceError, errorMessage);
				throw new Exception(errorMessage);
			}
		}
		...
		public bool IsCompatibleWith(PropertyType type)
		{
			return (this.Type == type
				...
				|| (this.Type == PropertyType.Float && type == PropertyType.Double)
				|| (this.Type == PropertyType.Double && type == PropertyType.Float)
				...
				|| (this.Type == PropertyType.FloatVector && type == PropertyType.DoubleVector)
				|| (this.Type == PropertyType.DoubleVector && type == PropertyType.FloatVector));
		}

		public List<PropertyValue> GetValuesAsList()
		{
			List<PropertyValue> retVal = new List<PropertyValue>();
