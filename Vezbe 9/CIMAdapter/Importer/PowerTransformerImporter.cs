...
namespace FTN.ESI.SIMES.CIM.CIMAdapter.Importer
{
    ...
    public class PowerTransformerImporter
    {
        ...
        private void ImportWindingTests()
        {
            ...
            if (cimWindingTests != null)
            {
                foreach (KeyValuePair<string, object> cimWindingTestPair in cimWindingTests)
                {
                    ...
                    ResourceDescription rd = CreateWindingTestResourceDescripton(cimWindingTest);
                    if (rd != null)
                    {
                        delta.AddDeltaOperation(DeltaOpType.Insert, rd, true);
                        report.Report.Append("Winding Test ID = ").Append(cimWindingTest.ID).Append(" SUCCESSFULLY converted to GID = ").AppendLine(rd.Id.ToString());
                    }
                    else
                    {
                        report.Report.Append("Winding Test ID = ").Append(cimWindingTest.ID).AppendLine(" FAILED to be converted");
                    }
                }
                report.Report.AppendLine();
            }
        ...
        private ResourceDescription CreateWindingTestResourceDescripton(FTN.WindingTest cimWindingTest)
        {
            ResourceDescription rd = null;
